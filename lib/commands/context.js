...
commands.setContext = async function (name, callback, skipReadyCheck) {
  ...
  // deal with performance logging, if requested
  if (this.opts.enablePerformanceLogging) {
    if (this.remote) {
    ...
    if (name === 'NATIVE_APP' && this.logs.performance) {
      this.logs.performance.stopCapture();
    }
  }
};
